...
export default function PedidosView() {
  ...
  // sub-estados en localStorage (opcional)
  ...
  // Estado detallado (backend + posible sub-estado)
  ...
  const obtenerEstiloEstado = (estado) => {
    switch (estado) {
      case "PENDIENTE_DUENO":
        return { texto: "Pendiente Dueño", estilo: styles.estadoPendiente };
      ...
      try {
        ...
        const pedidosNormalizados = (data || []).map((pd) => ({
          ...
          detalles: (pd.detalles || []).map((d) => ({
            ...
            tipo: typeof d.tipo === "string" ? d.tipo : d.tipo
          }))
        }));
        ...
        if (!isVendedor) {
          filtrados = filtrados.filter((p) => {
            ...
            return est !== "PENDIENTE_DUENO"; // El dueño excluye pedidos de vendedores
          });
        }
  ...
  const verificarStockAntesDeConfirmar = async () => {
    try {
      ...
      const data = await res.json();
      if (!res.ok || (data.insumos_insuficientes && data.insumos_insuficientes.length > 0)) {
  ...
  const confirmarPedido = async () => {
    ...
    if (!stockOk) return;
    try {
      let estadoInicial = user.rol === "Dueño" ? "APROBADO_DUENO" : "PENDIENTE_DUENO";
      const data = {
        usuario: user.id,
      ...
      if (!isVendedor) data.porcentaje_ganancia = formData.porcentajeGanancia;
      const res = await fetch("http://localhost:8000/api/pedidos/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const cancelarPedido = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showAlert(`❌ Error al cancelar pedido: ${err.error || "Desconocido"}`, "error");
      }
    } catch {
      showAlert("❌ Error al conectar con el servidor", "error");
    }
  ...
  const eliminarPedido = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        showAlert(`❌ Error al eliminar pedido: ${err.error || "Desconocido"}`, "error");
      }
    } catch {
      showAlert("❌ Error al conectar con el servidor", "error");
    }
  ...
  const actualizarEstadoPedido = async (id, nuevoEstado) => {
    try {
      const res = await fetch(`http://localhost:8000/api/pedidos/${id}/`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ Pedido_estado: nuevoEstado })
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  const verDetallesPedido = async (p) => {
    try {
      const res = await fetch(`http://localhost:8000/api/pedidos/${p.Pedido_ID || p.id}/`);
      if (!res.ok) {
        ...
        <div style={styles.contentWrapper}>
          <div style={styles.header}>
            <div style={styles.headerLeft}>
              ...
              <p style={styles.subtitle}>
                Gestión de pedidos - {isVendedor ? "Mostrando todos los pedidos (Vendedor)" : "Por defecto se muestran pedidos pendientes"}
              </p>
            </div>
            ...
            <select value={filtros.estado} onChange={(e) => setFiltros({ ...filtros, estado: e.target.value })} style={styles.select}>
              {isVendedor ? (
                <>
                  ...
                  <option value="PENDIENTE_DUENO">En Espera</option>
                  <option value="APROBADO_DUENO">Aceptado</option>
                </>
              ) : (
                <>
          ...
          {pedidosFiltrados.length > 0 ? (
              ...
              <table style={styles.table}>
                <thead>
                  <tr>
                    ...
                    {!isVendedor && <th style={styles.th}>Detalles</th>}
                ...
                <tbody>
                  {pedidosFiltrados.map((p) => {
                    ...
                    const puedeCancelar = (isVendedor && (p.usuario === user.id || p.Pedido_usuario_id === user.id) && estado === "PENDIENTE_DUENO") || (!isVendedor && (estado !== "COMPLETADO" && estado !== "CANCELADO" && estado !== "PENDIENTE_DUENO"));
                    return (
            ...
            <div style={styles.emptyState}>
              ...
              <p style={{ color: "#6b7280" }}>{isVendedor ? "No hay pedidos en el sistema actualmente." : "Usa el buscador para ver pedidos con otros estados o realiza uno nuevo."}</p>
            </div>
          )}
                ...
                {selectedPrenda && (
                  <div style={styles.formContainer}>
                    <div style={styles.formGrid}>
                      ...
                      <div>
                        <button onClick={agregarPrenda} style={{ ...styles.addBtn, gridColumn: isVendedor ? "span 1" : "auto" }}><Plus size={18} /> Agregar</button>
                      </div>
                    </div>
                  </div>
                )}
                      ...
                      <div key={idx} style={styles.pedidoCard}>
                        <div style={styles.pedidoInfo}>
                          ...
                          <div style={styles.pedidoDetalles}>
                            Talle: {p.talle} | Cantidad: {p.cantidad}{!isVendedor && ` | Precio unit: $${(p.precioUnitario || 0).toFixed(2)}`}
                          </div>
                        </div>

                        {!isVendedor && <div style={styles.pedidoPrecio}>{"$" + ((p.precioUnitario || 0) * (p.cantidad || 0)).toFixed(2)}</div>}
