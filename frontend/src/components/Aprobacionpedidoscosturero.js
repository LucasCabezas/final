...
import { CheckCircle, Package, PlayCircle, StopCircle, Send, AlertCircle, Clock, Scissors } from 'lucide-react';
...
const AprobacionPedidosCosturero = () => {
    ...
    const cargarPedidos = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${API_PEDIDOS_URL}?usuario_tipo=costurero`);
    ...
    // 🎨 ESTILOS COPIADOS DEL ESTAMPADOR Y ADAPTADOS PARA COSTURERO
    const styles = {
        container: {
            display: "flex", 
            minHeight: "100vh", 
                    ...
                    {pedidos.length === 0 ? (
                        <div style={styles.tableContainer}>
                            <div style={styles.emptyState}>
                                ...
                                <p>No tienes pedidos de costura por procesar en este momento.</p>
                            </div>
                        </div>
                    ) : (
                        <div style={styles.tableContainer}>
                    ...
                    <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div style={styles.modalBody}>
                            <div style={styles.modalInfo}>
                                ...
                                {selectedPedido.requiere_estampado && (
                                    <div style={styles.modalInfoItem}>
                                        <div style={styles.modalInfoLabel}>Observación</div>
                                        <div style={styles.modalInfoValue}>⚠️ Requiere estampado posterior</div>
                                    </div>
                                )}
                            </div>

                            {/* Sección destacada para prendas que requieren estampado */}
                            {selectedPedido.requiere_estampado && (
                                <div style={styles.estampadoSection}>
                                    <h4 style={styles.estampadoSectionTitle}>
                                        <Scissors size={20} />
                                        ⚠️ Este pedido requiere estampado posterior
                                    </h4>
                                    <p style={{ color: '#9ca3af', fontSize: '14px', margin: 0 }}>
                                        Una vez completada la costura, este pedido deberá ser trasladado al área de estampado.
                                    </p>
                                </div>
                            )}

                            <h4 style={{ color: '#fff', marginBottom: '16px' }}>Productos:</h4>
                            <div style={styles.tableContainer}>
                                <table style={styles.table}>
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        {selectedPedido.detalles.map((detalle, index) => {
                                            ...
                                            return (
                                                <tr key={index} style={esEstampada ? { backgroundColor: 'rgba(59, 130, 246, 0.1)' } : {}}>
                                                    ...
                                                    <td style={styles.td}>
                                                        <span style={{
                                                            ...
                                                            fontSize: '11px',
                                                            fontWeight: '600',
                                                            backgroundColor: esEstampada ? 'rgba(59, 130, 246, 0.2)' : 'rgba(107, 114, 128, 0.2)',
                                                            color: esEstampada ? '#3b82f6' : '#6b7280',
                                                            border: `1px solid ${esEstampada ? 'rgba(59, 130, 246, 0.3)' : 'rgba(107, 114, 128, 0.3)'}`,
                                                            display: 'flex',
                                                            alignItems: 'center',
                                                            gap: '4px'
                                                        }}>
                                                            {esEstampada && <Scissors size={12} />}
                                                            {detalle.tipo}
                                                        </span>
                                                    </td>
